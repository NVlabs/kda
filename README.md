...
This website introduces the program and directs contributors with prepared files to submit a pull request (PR) to `wishlist`. **Submit a request** opens a form for the contributor's fork and branch. **Continue to GitHub** opens the complete comparison against `NVlabs/kda:wishlist`, with the request template selected. **Discuss an idea** opens an optional GitHub issue for early discussion or help preparing the files. Request details live in the submitted README, so contributors do not have to repeat them in an issue or PR description.
...
2. Enter your fork and branch on the [submission page](https://nvlabs.github.io/kda/#submit). Continue to GitHub to review the comparison against `NVlabs/kda:wishlist` and fill in the [wishlist PR template](https://github.com/NVlabs/kda/blob/main/.github/PULL_REQUEST_TEMPLATE/wishlist.md); no issue or assigned number is needed. The form generates the link locally and does not upload files or create the PR. Keep the full details in the request README. Currently, KDA only supports NVIDIA B200 and B300 GPUs. If you need help preparing the materials, [discuss an idea](https://github.com/NVlabs/kda/issues/new?title=%5Bwishlist%5D%20) in a regular issue; complete files are optional for that discussion.
