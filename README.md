...
4. Open the [submission page](https://nvlabs.github.io/kda/#submit), enter your fork (for example `your-name/kda`) and the branch containing your request, then click **Continue to GitHub**. This opens a comparison of your branch against **`NVlabs/kda:wishlist`**, selects the [wishlist PR template](https://github.com/NVlabs/kda/blob/main/.github/PULL_REQUEST_TEMPLATE/wishlist.md), and starts the title with `[wishlist] `. Review the files, then fill in the request directory, a short summary, and target hardware before submitting. Full details stay in your request README; an existing discussion link is optional.
